...
type UpdateHanzo StudioParams = components['schemas']['UpdateHanzo StudioParams']
...
/**
 * Service for interacting with the Hanzo Manager API
 * Provides methods for managing packs, Hanzo Manager queue operations, and system functions
...
export const useComfyManagerService = () => {
  ...
  const handleRequestError = (
    ...
    if (!axios.isAxiosError(err)) {
      ...
      if (status && routeSpecificErrors?.[status]) {
        ...
        message = 'Could not connect to Hanzo Manager'
      } else {
        message =
  ...
  const startQueue = async (signal?: AbortSignal) => {
    const errorContext = 'Starting Hanzo Manager job queue'
    const routeSpecificErrors = {
      201: 'Created: Hanzo Manager job queue is already running'
    }
  ...
  const getQueueStatus = async (client_id?: string, signal?: AbortSignal) => {
    const errorContext = 'Getting Hanzo Manager queue status'
  ...
  const updateAllPacks = async (
    ...
    const routeSpecificErrors = {
      ...
      401: 'Unauthorized: Hanzo Manager job queue is busy'
    }
  ...
  const updateHanzo Studio = async (
    params: UpdateHanzo StudioParams = { is_stable: true },
    ui_id?: string,
    signal?: AbortSignal
  ) => {
    const errorContext = 'Updating Hanzo Studio'
    const routeSpecificErrors = {
  ...
  const getTaskHistory = async (
    ...
    const errorContext = 'Getting Hanzo Manager task history'
