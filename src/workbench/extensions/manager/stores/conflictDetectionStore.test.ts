...
describe('useConflictDetectionStore', () => {
  ...
  const mockConflictedPackages: ConflictDetectionResult[] = [
    {
      package_id: 'Hanzo Manager',
      package_name: 'Hanzo Manager',
      has_conflict: true,
      is_compatible: false,
      conflicts: [
        {
  ...
  describe('getConflictsForPackageByID', () => {
    it('should find package by exact ID match', () => {
      ...
      const result = store.getConflictsForPackageByID('Hanzo Manager')

      expect(result).toBeDefined()
      expect(result?.package_id).toBe('Hanzo Manager')
  ...
  describe('securityPendingPackages', () => {
    it('should filter packages with pending conflicts', () => {
      ...
      expect(securityPendingPackages[0].package_id).toBe('Hanzo Manager')
    })
  })
